...
public class TDOAObservationITs implements EfusionListener {
    ...
    public void testMoverNorthEast() throws Exception {
        ...
        target_a.setLat_move(+0.005); // MOVE NE
    ...
    @Test
    public void testStationaryTarget() throws Exception {
        target_a.setTrue_lat(-31.98); // BOTTOM (Demonstrates going to wrong local min with wrong init. Compare result with AOA_TDOA - which drifts off under same conditions)
        target_a.setTrue_lon(116.000);
    ...
    @Test
    public void testStationaryTarget_SingleMeasurement() throws Exception {
    ...
    @Test
    public void testMoverSouthWest() throws Exception {
        target_a.setTrue_lat(-31.7); // TOPRIGHT
        target_a.setTrue_lon(116.08);
        target_a.setLat_move(-0.005); // MOVE SW
        target_a.setLon_move(-0.005);
